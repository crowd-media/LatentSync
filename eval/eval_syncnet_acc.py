...
def main(config):
    ...
    for batch in test_dataloader:
        ...
        with torch.no_grad():
            torch.save(frames, os.path.join(save_folder_frames, f"frames_{global_step:04d}.pt"))
            torch.save(audio_samples, os.path.join(save_folder_audio, f"mel_chunk_{global_step:04d}.pt"))
            torch.save(y, os.path.join(save_folder_label, f"label_{global_step:04d}.pt"))
            vision_embeds, audio_embeds = syncnet(frames, audio_samples)

        sims = nn.functional.cosine_similarity(vision_embeds, audio_embeds)
        torch.save(sims, os.path.join(save_folder_sims, f"sims_{global_step:04d}.pt"))
